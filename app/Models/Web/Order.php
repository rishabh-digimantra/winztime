...
class Order extends Model
{
    ...
    public function place_order($request)
    {
        ...
        $delivery_firstname = auth()->guard('customer')->user()->first_name;
        $delivery_lastname = auth()->guard('customer')->user()->last_name;
        ...
        $delivery_company = "";
        $delivery = [];
        $country = DB::table('countries')->where('currency', '=', 'AED')->first();
        ...
        if (session('shipping_address')) {
            ...
            $country = DB::table('countries')->where('countries_id', '=', session('shipping_address')->countries_id)->first();
        }
        ...
        $delivery_country_id = $country ? $country->countries_id : null;
        $delivery_country = $country ? $country->countries_name : 'Dubai';
        $countries_iso_code_2  = $country ? $country->countries_iso_code_2 : 'UAE';
        $currency  = $country ? $country->currency : 'AED';
        $phonecode  = $country ? $country->phonecode : '971';
        $billing_firstname = auth()->guard('customer')->user()->first_name;
        $billing_lastname = auth()->guard('customer')->user()->last_name;
        ...
        if (session('billing_address')) {
            ...
            $country = DB::table('countries')->where('countries_id', '=', session('billing_address')->billing_countries_id)->first();
        }
        ...
        $billing_country = $country ? $country->countries_name : '0';
        ...
        if ($payment_status == 'success') {
            ...
            // Change shipping cost to 0 when delivery country is UAE
            if ( is_numeric( $delivery_country_id ) && intval( $delivery_country_id ) === 231 ) {
                $shipping_cost = 0;
            }
            ...
            if ($request->totalCouponChecker == 'added') {
                ...
                DB::table('orders')->where('orders_id', $orders_id)->update([
                    ...
                    'billing_state' => '',
                    'billing_country' => '',
                    'delivery_phone' => auth()->guard('customer')->user()->country_code . ' ' . auth()->guard('customer')->user()->phone,
                    'billing_phone' => auth()->guard('customer')->user()->country_code . ' ' . auth()->guard('customer')->user()->phone,
                ]);
            }
            ...
            $extra_l_point = @$request->order_lpoints ? $request->order_lpoints : 0;
            if (isset($request->remaining_point)) {
                // echo '1----';
                $redeem_points = $request->remaining_point + $extra_l_point;
            } else {
                // echo '2----';
                $redeem_points = auth()->guard('customer')->user()->redeem_points + $extra_l_point;
            }
            $level = Level::where('start_count', '<=', $redeem_points)->where('end_count', '>=', $redeem_points)->value('id');
            // echo $redeem_points .'<br>';
            // echo $level .'<br>';
            // die('test');
            $a = DB::table('users')->where('id', auth()->guard('customer')->user()->id)->update(['level_id' => $level]);
            ...
            foreach ($cart_items as $products) {
                ...
                $WinnerController->campaignExpire($products->campaign_id[0]->id);
            }
